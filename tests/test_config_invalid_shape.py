...
record_typo = true''',
    ...
    def test_required_top_level_values_and_tables_are_reported(self) -> None:
        cases = (
            (replace_once(STARTER_POLICY_TEXT, "version = 1\n\n", ""), "version"),
            (
                replace_once(
                    STARTER_POLICY_TEXT,
            ...
            (
                STARTER_POLICY_TEXT[
                    : STARTER_POLICY_TEXT.index("[[file_rule]]\n")
                ]
                + STARTER_POLICY_TEXT[STARTER_POLICY_TEXT.index("[[path_override]]\n") :],
                "file_rule",
            ),
        )
    ...
    def test_required_keys_are_reported_in_fixed_and_repeatable_records(self) -> None:
        cases = (
            ...
            (replace_once(STARTER_POLICY_TEXT, 'roots = ["docs/index.md"]\n', ""), "documentation.roots"),
            (
                replace_once(
                    STARTER_POLICY_TEXT,
            ...
            (replace_once(STARTER_POLICY_TEXT, 'name = "bootstrap"\n', ""), "context_set[0].name"),
            (replace_once(STARTER_POLICY_TEXT, "compare_file_sizes = true\n", ""), "ratchet.compare_file_sizes"),
            (replace_once(STARTER_POLICY_TEXT, "require_reason = true\n", ""), "exceptions.require_reason"),
        )
        for text, field_path in cases:
            with self.subTest(field_path=field_path):
                self.assert_policy_diagnostic(text, CFG_MISSING_KEY, field_path)
...
scan = false''',
    ...
    def test_scalar_and_array_types_are_strict(self) -> None:
        cases = (
            (replace_once(STARTER_POLICY_TEXT, "version = 1", "version = true"), "version"),
            (
                replace_once(STARTER_POLICY_TEXT, 'inventory = "git-visible"', "inventory = 1"),
                "repository.inventory",
            ),
            (
                replace_once(STARTER_POLICY_TEXT, "stable_sort = true", 'stable_sort = "true"'),
                "output.stable_sort",
            ),
            (
                replace_once(
            ...
            (
                replace_once(STARTER_POLICY_TEXT, "hard_bytes = 25600", "hard_bytes = true"),
...
scan = false''',
    ...
    def test_unsupported_version_and_enum_values_are_rejected(self) -> None:
        cases = (
            (replace_once(STARTER_POLICY_TEXT, "version = 1", "version = 2"), "version"),
            (
                replace_once(STARTER_POLICY_TEXT, 'inventory = "git-visible"', 'inventory = "filesystem"'),
                "repository.inventory",
            ),
            (
                replace_once(STARTER_POLICY_TEXT, 'encoding = "utf-8"', 'encoding = "utf-16"'),
                "repository.encoding",
            ),
            (
                replace_once(STARTER_POLICY_TEXT, 'kind = "generated"', 'kind = "binary"'),
                "file_rule[0].kind",
            ),
            (
                replace_once(STARTER_POLICY_TEXT, 'default_format = "text"', 'default_format = "json"'),
                "output.default_format",
            ),
        )
        for text, field_path in cases:
