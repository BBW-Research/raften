...
def build_zipapp(source_root: Path, destination: Path, *, epoch: int) -> tuple[str, ...]:
    ...
    license_path = source_root.parent / "LICENSE"
    if not license_path.is_file() or license_path.is_symlink():
        raise ValueError(f"source tree does not contain a regular LICENSE: {license_path}")
    ...
    entries = {"LICENSE": license_path.read_bytes(), "__main__.py": _MAIN}
