...
- [ ] Implement repository-root validation independent of package location and current working directory.
- [ ] Implement Git-visible inventory using argument-vector subprocess calls and NUL-delimited output.
- [ ] Represent tracked, untracked, deleted, regular, symlink, and missing states explicitly where relevant.
- [ ] Add base-revision blob and metadata access without checking out or modifying the worktree.
- [ ] Implement canonical POSIX path normalization on macOS, Linux, and Windows.
- [ ] Implement the specified repository glob language. `*` and `?` must not cross `/`; `**` must. Do not call raw `fnmatch` outside the matcher module.
- [ ] Add matcher tests for root files, nested files, dotfiles, Unicode, character classes, repeated separators, `**`, case sensitivity, and malformed patterns.
- [ ] Sort inventory and all derived collections explicitly.
- [ ] Handle files that disappear during a run with a deterministic operational diagnostic.

### Gate

- [ ] Inventory works when the CLI is launched outside the target repository.
- [ ] Fixture tests cover tracked and non-ignored untracked files, ignored files, deletion, rename, symlinks, and base blobs.
- [ ] Matcher behavior is completely defined by tests and documentation.
- [ ] No code outside `inventory.py` invokes Git; no code outside `matcher.py` interprets policy globs.
