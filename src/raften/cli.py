...
from raften.diagnostics import INT_INTERNAL, operational_diagnostic
from raften.model import OutputFormat
from raften.report import (
    command_exit_code,
    render_emergency_failure,
    render_report,
)
from raften.run_model import CommandFailure, RunStatus
from raften.runner import explain_repository_path, initialize_repository, run_repository
...
def build_parser() -> argparse.ArgumentParser:
    parser = _SafeArgumentParser(
        prog="raften",
        description=(
            "Check code and documentation budgets, documentation reachability, "
            "and policy ratchets against Git history."
        ),
    )
    ...
    init.add_argument(
        "--config",
        default="raften.toml",
        help="Repository-relative destination policy path (default: raften.toml).",
    )
    init.add_argument(
        "--capture-debt",
        action="store_true",
...
def _add_repository_options(
    ...
    *,
    ...
    parser.add_argument(
        "--config",
        default="raften.toml",
        help="Repository-relative policy path (default: raften.toml).",
    )
